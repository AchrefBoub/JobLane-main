          ...
          <form onSubmit={registerHandler} className='flex flex-col md:w-1/3 shadow-gray-700  w-full md:mx-0 mx-8' action="">
            ...
            <div className='md:px-10 px-2 pt-4 pb-20 w-full flex flex-col gap-4'>
              <div className='text-center'>
                <p className='text-4xl  font-medium'>التسجيل</p>
              </div>
              ...
              <div className='bg-white flex justify-center items-center'>
                ...
                <input value={password} onChange={(e) => setPassword(e.target.value)} required placeholder='كلمة المرور' type={eyeTog ? "text" : "password"} className='outline-none bold-placeholder w-full text-black px-1 pr-3 py-2' />
              ...
              <div>
                <div className='bg-white flex justify-center items-center'>
                  ...
                  <label htmlFor='avatar' className='outline-none w-full cursor-pointer text-black px-1 pr-3 py-2 '>
                    {avatarName.length === 0 ? <span className='text-gray-500 font-medium'>اختر صورة الملف الشخصي...</span>
                      : avatarName}
                  </label>
                  <input id='avatar' name='avatar' required
                         onChange={avatarChange}
                         placeholder='الملف الشخصي' accept="image/*" type="file" className='outline-none  w-full hidden text-black px-1 pr-3 py-2' />
                </div>
              ...
              <div>
                <div className='bg-white flex justify-center items-center'>
                  ...
                  <label className='outline-none w-full text-black px-1 pr-3 py-2' htmlFor="resume">
                    {resumeName.length === 0 ? <span className='text-gray-500 cursor-pointer font-medium'>اختر السيرة الذاتية...</span> : resumeName}
                  </label>
                  <input required
              ...
              <div className='bg-white flex justify-center items-center'>
                ...
                <textarea value={skills} onChange={(e) => setSkills(e.target.value)} placeholder='المهارات' type="text" className='outline-none w-full text-black bold-placeholder px-1 pr-3 py-2' />
              </div>

              <div>
                <button disabled={loading} className='blueCol flex justify-center items-center px-8 w-full py-2 font-semibold' >
                  {loading ? <TbLoader2 className='animate-spin' size={24} /> : "تسجيل"}</button>
              </div>

              <div className='text-center text-sm pt-2'>
                <p>لديك حساب بالفعل،<Link to="/login" className='text-yellow-400 underline'>تسجيل الدخول</Link> من هنا. </p>
              </div>
