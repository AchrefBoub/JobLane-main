...
export const JobDetails = () => {
  ...
  const notLoginHandler = (str) => {
    if (!isLogin) {
      toast.info(`الرجاء تسجيل الدخول لـ ${str} الوظيفة`);
      navigate("/login");
    }
  };
            ...
            {jobDetails &&
              <div>
                ...
                <div className='md:px-12 pl-4'>
                  <div>
                    <p className='text-2xl py-3 '>التفاصيل:</p>
                  </div>
                  <div>
                    <ul className='flex flex-col gap-3'>
                      <li className='flex items-center gap-3'>المعلن: <div>{jobDetails.postedBy.name}</div></li>
                      <li className='flex items-center gap-3'>النشر في: <div>{convertDateFormat(jobDetails.createdAt.substr(0, 10))}</div></li>
                      <li className='flex items-center gap-3'>الموقع: <div> {jobDetails.location}</div></li>
                      <li className='flex items-center gap-3'>الراتب: <div className='flex items-center' ><BiRupee />  <span>{jobDetails.salary} LPA</span></div></li>
                      <li className='flex items-center gap-3'>الخبرة المطلوبة: <div> {jobDetails.experience}</div></li>
                      <li className='flex items-center gap-3'>المهارات المطلوبة: <div className='flex flex-wrap items-center gap-3'> {jobDetails.skillsRequired.map((e, i) => (<span key={i} className='px-2 py-0.5 bg-yellow-600 rounded text-black md:text-sm font-semibold text-xs'>{e}</span>))}</div></li>
                      <li className='grid gird-cols-1 gap-2 pt-2'><div className='text-2xl'>وصف الوظيفة: </div> <div> {jobDetails.description}</div></li>
                    </ul>
                  </div>
                </div>
                <div className='md:px-12 pl-4 flex gap-8 pb-32 pt-6 '>
                  <button
                    onClick={() => {
                      isLogin ?
                        me.appliedJobs && me.appliedJobs.includes(jobDetails._id) ? toast.error("لقد قدمت بالفعل!") :
                          navigate(`/Application/${jobDetails._id}`)
                        :
                        notLoginHandler("تقديم")
                    }}
                    className=' hover:bg-green-600 md:text-lg text-sm font-bold px-10 py-1.5 bg-green-800 flex items-center gap-1 '>
                    <BsSend /> {me.appliedJobs && me.appliedJobs.includes(jobDetails._id) ? "تم التقديم" : "تقديم"}
                  </button>
                  <button
                    onClick={() => {
                      if (isLogin) {
                    ...
                    {saveJobLoading ?
                      <span className='animate-spin px-5'><TbLoader2 size={20} /></span> :
                      <>
                        <AiOutlineSave />
                        {
                          me.savedJobs && me.savedJobs.includes(jobDetails._id) ? "إلغاء الحفظ" : "حفظ"
                        }
                      </>
                    }
                  </button>
